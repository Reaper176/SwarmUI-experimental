...
**Status:** Implemented and maintainer-validated on Linux
...
Task specification and code-quality review both passed. Source-only static review confirmed the exact one-file production range, lifecycle/continuation inputs, cleanup ordering, caller/selected-pressure coupling, unchanged public signatures, unchanged `Session.cs`, and clean fixed-range whitespace. In accordance with repository policy, agents performed no build, test, launch, model load, cancellation injection, runtime validation, platform validation, or performance measurement. Maintainer Reaper176 confirmed the complete matrix below passed on Linux on 2026-07-25. Windows and other-platform runtime behavior and performance remain unvalidated.
...
The maintainer built and ran the live software on Linux. For every case, the matrix identified both the scheduling caller/token owner and the globally selected pressure and observed both pressure counts, selected `IsLoading`, selected session claims/counters, loader reservation, selected failure/retry inputs, caller refusal/`Failure` delivery, emitted cleanup-failure diagnostics or intentionally injected diagnostic failures, and later progress. Baseline-restoration expectations in ordinary cases assume cleanup primitives do not themselves throw; dedicated cleanup- and diagnostic-failure cases verified the bounded best-effort behavior.
...
Maintainer Reaper176 confirmed all 20 cases passed on Linux on 2026-07-25. Publication/setup failures after `IsLoading`, after captured claims, at `StartNew`, and during continuation attachment preserved the original exception and selected exactly one cleanup owner, including the attachment/delegate-entry race. Pre-start cancellation passed for sole and shared selected pressure and for matching and different scheduling-token owners, restoring published selected state when primitives succeeded without backend classification, refresh, caller-pressure mutation, or refusal/failure delivery.

Started-path validation passed for matching and different token owners while the loader was busy, busy `(none)` selection, global shutdown while waiting for usage, cancellation after `LoadModel` began, and global shutdown during `LoadModel` with the scheduling token both canceled and uncanceled. The existing busy-loop and load-wait token timing remained unchanged; the started owner ran once, released its reservation, restored selected state and claims when primitives succeeded, retained selected-pressure failure/retry ownership, and allowed later progress.

Compatibility validation passed for ordinary and `(none)` success with matching and different owners, readable failure and model mismatch, all-loaders-failed behavior with non-null caller pressure/input, null input, and null caller pressure with non-null input, repeated dispatch-race cancellation, and normal sole/shared and competing-model generation. Pressure ordering, caller-local status/refusal/`Failure` delivery, selected loader/model and retry/detail ownership, final images, subsequent requests, and shutdown behavior remained compatible, with no negative counters, duplicate classification, or permanent selected `IsLoading`.

Cleanup-failure injection passed for pre-start `IsLoading` reset; early, middle, and late claim disposal; and started reservation release, model comparison, mismatch warning/classification, debug logging, `IsLoading` reset, claim disposal, and model-list refresh. Later independent steps continued; each captured claim received at most one Rank 11 cleanup-owner disposal attempt; disposal failure was followed by one suppression attempt; partial disposal followed by successful suppression retained the documented partial-state/resource caveat without a later finalizer disposal; and suppression failure retained the documented possibility of a later finalizer-invoked `Dispose`.

Diagnostic-failure injection passed independently for diagnostic context/message construction, `ReadableString()`, and `Logs.Error` during setup failure, pre-start cancellation, disposal/suppression failure, and started cleanup. The original exception remained unchanged, cancellation cleanup did not fault, required suppression still ran, and later cleanup continued. These Linux results validate the recorded runtime concurrency, counter, backend, failure-delivery, and diagnostic outcomes; they do not validate Windows or other-platform behavior or performance. Agents performed no runtime/build/test. The constructor-before-return limitation, partial disposal/resource caveat, suppression-failure finalizer caveat, and the four adjacent non-goals remain unchanged.

...
| A later claim construction/capture or setup step throws before a task starts | No | Outer setup catch | Acquire `PreStartCleanupOwned`; make one cleanup-owner attempt for `IsLoading`, each captured disposal, and post-failure suppression where needed; rethrow original | Cleanup failures are logged/continued and never replace original |
...
| Captured `GenClaim.Dispose` throws | Either | Owning cleanup function | Log disposal; the Rank 11 cleanup owner does not call `Dispose` again; separately attempt `GC.SuppressFinalize(claim)` once; then continue captured order | Partial claim/resource/session state may remain; successful suppression prevents a finalizer-invoked `Dispose` but does not repair state |
| Post-failure `GC.SuppressFinalize` throws | Either | Owning cleanup function | Log suppression failure; the Rank 11 cleanup owner does not retry disposal or suppression; continue later claims/steps | A later finalizer-invoked `Dispose` and repeated partial mutation cannot be ruled out |
| Any other cleanup primitive throws | Either | Owning cleanup function | Catch/log server-side; continue later steps/claims; the Rank 11 cleanup owner does not retry the failed primitive | Terminal state records ownership, not successful restoration |
...
2. Implement non-throwing pre-start cleanup as isolated ordered attempts: selected `IsLoading` reset, then one `Dispose` attempt per captured claim. After disposal failure, log and separately attempt `GC.SuppressFinalize(claim)` once before advancing; log suppression failure, and have the Rank 11 cleanup owner retry neither call. A suppression failure leaves a later finalizer-invoked `Dispose` possible.
...
12. prove a throwing `GenClaim.Dispose` is logged, not retried by the Rank 11 cleanup owner, followed by one non-throwing `GC.SuppressFinalize(claim)` attempt, and cannot prevent later claims/refresh;
13. prove suppression failure is logged and not retried by the Rank 11 cleanup owner, later work continues, and a later finalizer-invoked `Dispose` is explicitly left possible;
...
16. **Pre-start cleanup primitive failures:** inject selected `IsLoading` and early/middle/late disposal failures. Confirm the Rank 11 cleanup owner calls each claim's `Dispose` once, follows each failed disposal with one suppression attempt, continues later claims in captured order, preserves the original setup/attachment exception, and does not fault the cancellation continuation.
17. **Started cleanup primitive failures:** inject reservation, mismatch, `IsLoading`, disposal, and refresh failures. Confirm each failed cleanup-owner disposal receives one suppression attempt, later claims/refresh continue in order, logs identify each failure, and no cleanup-owner call is retried.
...
19. **Suppression failure:** make disposal throw and the subsequent suppression attempt throw. Confirm both failures are logged, neither cleanup-owner call is retried, later claims/steps continue, and validation explicitly records that a later finalizer-invoked `Dispose` cannot be ruled out.
...
- a failed `GenClaim.Dispose` is not retried by the Rank 11 cleanup owner and receives one separate non-throwing `GC.SuppressFinalize(claim)` attempt before later claims;
...
- suppression failure is logged and not retried by the Rank 11 cleanup owner, while a later finalizer-invoked `Dispose` remains explicitly possible;

...
This guarantee covers every `Session.GenClaim` instance that construction successfully returns and the implementation captures. It cannot transactionally recover counters or other mutations if `Session.GenClaim` construction itself throws internally before returning the instance to this owner. It also does not claim transactional recovery if a cleanup primitive itself throws: the cleanup owner logs that server-side failure and continues attempting every later independent step, but it neither retries nor claims restoration for the failed primitive.
...
Both cleanup functions must be non-throwing. Each independent ordered action, and each individual captured `GenClaim.Dispose`, runs inside its own try/catch. A cleanup exception is logged with enough server-side owner/step context to diagnose it, then cleanup continues with the next ordered action or claim. Pre-start cleanup therefore cannot replace the original publication/setup/attachment exception, and the cancellation continuation cannot fault because one cleanup primitive failed.
...
2. call `Dispose` once on every successfully returned and captured model-load claim, in captured order.

The `IsLoading` attempt and every individual claim-disposal attempt are isolated by separate try/catch blocks. A failure is logged server-side and later claims are still attempted. A failed claim is never retried.
...
5. call `Dispose` once on every captured model-load claim, in captured order; and
6. call `ReassignLoadedModelsList`.

Reservation release, mismatch classification, `IsLoading` reset, each individual claim disposal, and model-list reassignment are isolated from one another with per-step try/catch. If any attempt throws, cleanup logs the step failure server-side and continues to every later action. The existing load-exception path remains responsible for unwrapping aggregate exceptions, logging the readable failure, and adding its readable text to `BackendFailReasons`. The cleanup owner does not change which load exception types are caught, the messages recorded, or the mismatch rule.

`Session.GenClaim.Dispose` is not idempotent: it subtracts its current counters before removing the claim and disposing its cancellation source, and an exception can occur after a partial mutation. The exclusive ownership state therefore permits at most one `Dispose` call per captured instance. A thrown disposal is logged and not retried; cleanup continues to later claims and steps. No transactional repair or baseline guarantee is made for that failed claim.
...
| A later claim construction/capture or setup step throws before a task starts | No | Outer setup catch | Acquire `PreStartCleanupOwned`; non-throwingly attempt `IsLoading` reset and each captured disposal once; rethrow original exception | Cleanup failures are logged/continued and never replace original; no guarantee for constructor failure before return or a failed primitive |
...
| Scheduling caller belongs to selected pressure; its token cancels task before delegate entry | No | Non-cancelable `OnlyOnCanceled \| ExecuteSynchronously` continuation on `TaskScheduler.Default` | Acquire `PreStartCleanupOwned`; non-throwingly attempt selected `IsLoading` reset and each captured disposal once | Cleanup failure is logged and continuation does not fault; no load classification/refresh |
...
| Any cleanup primitive throws | Either | Owning cleanup function | Catch/log server-side; continue later steps/claims; never retry failed primitive or claim | Terminal state records exclusive ownership, not successful restoration; cleanup remains non-throwing |
| Any losing cleanup invocation or repeated invocation | Either | Any of the three callers | Interlocked transition fails and returns without mutation | No second attempt of any claim/step, including one that failed during the owning attempt |
...
Because `GenClaim.Dispose` can mutate counters before a later operation throws, retrying the same instance could subtract twice. The owner records terminal cleanup ownership before attempts begin, visits each captured claim once, catches/logs any disposal exception, and advances. Neither a losing cleanup caller nor the owning function retries a failed claim or other failed primitive.

Non-throwing cleanup preserves the surrounding exception boundary. Publication/setup/attachment catches rethrow their original exception after best-effort pre-start cleanup, never a cleanup exception. The cancellation-only continuation completes without a cleanup fault. Started cleanup does not abandon later state attempts because reservation release, mismatch classification, `IsLoading`, one claim, or refresh threw.
...
2. Implement non-throwing pre-start cleanup as isolated ordered attempts: selected `IsLoading` reset, then one `Dispose` attempt per captured claim. Log each failure, continue, and never retry a failed claim.
...
4. Implement non-throwing started cleanup as isolated ordered attempts: reservation release, mismatch classification, selected `IsLoading` reset, one `Dispose` attempt per captured claim, and `ReassignLoadedModelsList`. Log each failure and continue; terminal ownership never implies primitive success.
...
9. prove pre-start cleanup non-throwingly attempts only selected `IsLoading` reset and each captured claim once, with separate try/catch/log/continue behavior per action;
...
11. prove started cleanup non-throwingly attempts reservation release, selected mismatch classification, selected `IsLoading` reset, each captured claim once, and `ReassignLoadedModelsList` in that order, with separate try/catch/log/continue behavior per action;
12. prove a throwing `GenClaim.Dispose` is logged, never retried, and cannot prevent attempts for later claims or refresh; do not claim repair for its possible partial mutation;
13. separately trace scheduling-caller token owner equal to and different from selected pressure for pre-start cancellation, busy waiting, load cancellation/success/failure, selected retry/detail inputs, and caller refusal/`Failure` delivery;
14. separately trace global shutdown while waiting for usages and during `LoadModel` with and without cancellation of the scheduling-caller token;
15. prove the busy loop still observes only `Program.GlobalProgramCancel`, while the load wait still observes only scheduling-caller `cancel`;
16. prove load exception unwrapping/logs, selected `BackendFailReasons`/`BadBackends`, retry/exclusion, and detailed-exception inputs remain unchanged;
17. prove cleanup invokes no `releasePressure`, generic refusal, all-loaders exception delivery, caller `Failure`, or unrelated-pressure classification;
18. prove all-loaders-failed retains exact callback conditions/order when `highestPressure` differs;
19. trace the detailed exception from selected inputs through caller `TryFind`, caller `Failure`, and caller waiter; prove no selected exception state;
20. prove caller-local notification/refusal/`Failure` delivery and selected session/failure/retry/detail-input ownership remain unchanged;
21. prove original scheduling-caller token/scheduler behavior remains on `StartNew` and `Wait(cancel)`;
22. prove four-argument cleanup continuation inputs remain exact and `ExecuteSynchronously` is only preferred;
23. prove pressure ordering/filtering, selection, reservation acquisition, wait cadence, successful loading, scheduler loop, and public API/ABI remain unchanged;
24. confirm no change to `LoadModelOnAll`, general `Session.GenClaim` semantics, browser code, launchers, extensions, generated content, user data, or adjacent findings;
25. inspect the exact commit/file range; and
...
16. **Pre-start cleanup primitive failures:** individually inject failure in selected `IsLoading` reset and in early/middle/late claim disposal during setup failure and pre-start cancellation. Confirm each failure is logged, later claims are attempted, no claim is retried, the original setup/attachment exception remains the thrown exception, and the cancellation continuation does not fault.
17. **Started cleanup primitive failures:** individually inject failure in reservation release, mismatch classification, selected `IsLoading` reset, an early/middle/late claim disposal, and `ReassignLoadedModelsList`. Confirm ordered later attempts still occur, each failure is logged server-side, and no failed step/claim is retried.
18. **Partial `GenClaim.Dispose` failure:** inject disposal failure after an observable partial mutation. Confirm the claim is attempted once only, later claims and refresh are still attempted, terminal ownership remains exclusive, and records make no baseline/transactional-repair claim for that failed claim.
...
- each ordered step and captured claim is attempted by at most one owner and at most once;
...
- a failed `GenClaim.Dispose` is never retried, and no transactional/baseline guarantee is made for its possible partial mutation;
...
Rollback is one bounded ownership change in `BackendHandler.LoadHighestPressureNow`: remove the `PreStartCleanupOwned`/`StartedCleanupOwned` lifecycle gate, captured-claims owner, non-throwing per-step/per-claim cleanup functions and logs, outer setup catch, delegate-entry arbitration, and cancellation-only continuation; restore the original claim publication, `Task.Factory.StartNew(action, cancel)`, delegate-local cleanup block, and post-finally `ReassignLoadedModelsList`.

No data migration, configuration rollback, public API restoration, browser change, extension rebuild contract, or scheduling-token/selected-pressure realignment is involved. Existing token, callback, refusal, selected failure-input, and caller failure-delivery ownership remains unchanged. A partial rollback is invalid: retaining multiple cleanup callers without exclusive ownership can double-attempt non-idempotent claims, while retaining terminal ownership without non-throwing ordered attempts can let one primitive failure suppress later cleanup or replace the original setup exception.

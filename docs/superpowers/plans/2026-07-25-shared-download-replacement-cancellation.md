...
- `loadData` exclusively owns each active response/stream pair and disposes it during retry handoff or in its `finally`; `saveChunks` and `sendUpdates` do not consume those resources, and the outer `using` declarations later repeat safe disposal of the initial pair.

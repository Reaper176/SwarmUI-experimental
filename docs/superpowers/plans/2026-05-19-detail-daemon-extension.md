# Detail Daemon Extension Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a portable SwarmUI extension that exposes Jonseed's Detail Daemon sampler in Swarm's normal Generate UI and installs the required ComfyUI node pack.

**Architecture:** Add a self-contained extension under `src/Extensions/SwarmUI-DetailDaemon/`. The extension registers a `detail_daemon` Comfy feature, defines a toggleable advanced parameter group, and rewrites eligible `KSamplerAdvanced`/`SwarmKSampler` workflow nodes into `SamplerCustomAdvanced` chains that wrap the selected sampler with `DetailDaemonSamplerNode`.

**Tech Stack:** C#/.NET SwarmUI extension APIs, Newtonsoft.Json `JObject`/`JArray`, SwarmUI Comfy backend workflow generator, Markdown docs.

---

## File Structure

- Create `src/Extensions/SwarmUI-DetailDaemon/SwarmUI-DetailDaemon.csproj`: portable extension project file using SwarmUI's shared extension props.
- Create `src/Extensions/SwarmUI-DetailDaemon/DetailDaemonExtension.cs`: extension registration, parameters, feature detection, and workflow rewriting.
- Create `src/Extensions/SwarmUI-DetailDaemon/README.md`: user-facing install and usage docs.
- No Swarm core files are modified.

## Task 1: Scaffold the Portable Extension

**Files:**
- Create: `src/Extensions/SwarmUI-DetailDaemon/SwarmUI-DetailDaemon.csproj`
- Create: `src/Extensions/SwarmUI-DetailDaemon/README.md`

- [ ] **Step 1: Create the extension project file**

Create `src/Extensions/SwarmUI-DetailDaemon/SwarmUI-DetailDaemon.csproj` with exactly:

```xml
<Project Sdk="Microsoft.NET.Sdk.Web">
    <PropertyGroup>
        <AssemblyName>SwarmUI-DetailDaemon</AssemblyName>
    </PropertyGroup>
    <Import Project="../../SwarmUI.extension.props" />
</Project>
```

- [ ] **Step 2: Create the initial README**

Create `src/Extensions/SwarmUI-DetailDaemon/README.md` with exactly:
...
```bash
ls src/Extensions/SwarmUI-DetailDaemon
```
...
```bash
git add src/Extensions/SwarmUI-DetailDaemon/SwarmUI-DetailDaemon.csproj src/Extensions/SwarmUI-DetailDaemon/README.md
git commit -m "Add Detail Daemon extension scaffold"
```

## Task 2: Register Feature and UI Parameters

**Files:**
- Create: `src/Extensions/SwarmUI-DetailDaemon/DetailDaemonExtension.cs`

- [ ] **Step 1: Add extension class with feature registration and parameters**

Create `src/Extensions/SwarmUI-DetailDaemon/DetailDaemonExtension.cs` with exactly:
...
```bash
dotnet build src/Extensions/SwarmUI-DetailDaemon/SwarmUI-DetailDaemon.csproj
```

Expected: build succeeds with `0 Error(s)`. Existing repository warnings are acceptable.
...
```bash
git add src/Extensions/SwarmUI-DetailDaemon/DetailDaemonExtension.cs
git commit -m "Register Detail Daemon extension parameters"
```

## Task 3: Add Workflow Sampler Rewriting

**Files:**
- Modify: `src/Extensions/SwarmUI-DetailDaemon/DetailDaemonExtension.cs`

- [ ] **Step 1: Add workflow hook call**

In `OnInit`, directly after `ComfyUIBackendExtension.NodeToFeatureMap["DetailDaemonSamplerNode"] = FeatureId;`, add:

```csharp
        WorkflowGenerator.AddStep(ApplyDetailDaemon, 9);
```

- [ ] **Step 2: Add workflow helper methods**

Inside `DetailDaemonExtension`, after `OnInit`, add exactly:
...
- [ ] **Step 3: Add missing namespace import**

At the top of `DetailDaemonExtension.cs`, add these imports if they are not already present:
...
- [ ] **Step 5: Commit workflow rewriting**

Run:

```bash
git add src/Extensions/SwarmUI-DetailDaemon/DetailDaemonExtension.cs
git commit -m "Wrap Swarm samplers with Detail Daemon"
```

## Task 4: Verify Extension Discovery in Full Build

**Files:**
- No source edits expected

- [ ] **Step 1: Build the SwarmUI source project**
...
Expected: build succeeds with `0 Error(s)`. Existing warnings are acceptable.

- [ ] **Step 2: Confirm extension build output exists**

Run:

```bash
find src -path '*SwarmUI-DetailDaemon*' -type f | sort
```

Expected output includes the source files and build artifacts for `SwarmUI-DetailDaemon`.
...
Expected: only intentional source files are tracked or modified. Ignore pre-existing unrelated `?? .superpowers/` if it is still present.

## Task 5: Final Documentation Pass and Verification

**Files:**
- Modify: `src/Extensions/SwarmUI-DetailDaemon/README.md` only if verification changes usage instructions

- [ ] **Step 1: Re-read README for accuracy**

Run:

```bash
sed -n '1,220p' src/Extensions/SwarmUI-DetailDaemon/README.md
```

Expected: README states that the extension rewrites only normal `KSamplerAdvanced` and `SwarmKSampler` nodes and requires the upstream Comfy node pack.

- [ ] **Step 2: Run final extension build**
...
```bash
git diff HEAD -- src/Extensions/SwarmUI-DetailDaemon
```

Expected: no unstaged source changes if all task commits were made. If README was adjusted in this task, review the diff and commit it with:

```bash
git add src/Extensions/SwarmUI-DetailDaemon/README.md
git commit -m "Document Detail Daemon extension usage"
```
...
```text
Verified: dotnet build src/Extensions/SwarmUI-DetailDaemon/SwarmUI-DetailDaemon.csproj
Verified: dotnet build src/SwarmUI.csproj
```

If either command cannot be run because the environment is missing dependencies, record the exact command and the blocking error.

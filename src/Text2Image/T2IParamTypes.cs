...
public class T2IParamTypes
{
    ...
    public static void RegisterDefaults()
    {
        ...
        DenoiseStrength = Register<double>(new("Denoise Strength", "Text-to-image denoise strength, as a percentage of steps to actually run.\nAt 100%, sampling starts from the first step as normal.\nAt 60% with Steps=20, sampling skips the first 8 steps and runs the final 12.\nThis only applies when no Init Image is selected. Init Image uses Init Image Creativity instead.",
            "100", Min: 0, Max: 100, Step: 5, Examples: ["50", "60", "80", "100"], OrderPriority: -16, ViewType: ParamViewType.SLIDER, Group: GroupCore, ChangeWeight: -3, CanSectionalize: true
            ));
        // ================================================ Text2Video ================================================
        ...
        AspectRatio = Register<string>(new("Aspect Ratio", "Image aspect ratio - that is, the shape of the image (wide vs square vs tall).\nSet to 'Custom' to define a manual width/height instead.\nSome models can stretch better than others.\nNotably Flux models support almost any resolution you feel like trying.",
            "1:1", GetValues: (_) => [
                ...
                "4:1///4:1", "3.88:1///31:8", "3.75:1///15:4", "3.63:1///29:8", "3.11:1///28:9", "3:1///3:1", "2.89:1///26:9", "2.5:1///5:2", "2.4:1///12:5", "2.09:1///23:11", "2:1///2:1", "1.91:1///21:11", "1.75:1///7:4",
                "1.67:1///5:3", "1.46:1///19:13", "1.38:1///18:13", "1.29:1///9:7", "1.21:1///17:14", "1.13:1///17:15", "1.07:1///16:15",
                "1:1.07///15:16", "1:1.13///15:17", "1:1.21///14:17", "1:1.29///7:9", "1:1.38///13:18", "1:1.46///13:19", "1:1.67///3:5", "1:1.75///4:7", "1:2///1:2", "1:2.09///11:23", "1:2.4///5:12", "1:2.5///2:5",
                "1:2.89///9:26", "1:3///1:3", "1:3.11///9:28", "1:3.63///8:29", "1:3.75///4:15", "1:3.88///8:31", "1:4///1:4",
                "Custom"
        ...
        MaskShrinkGrowResolutionOverrideMP = Register<double>(new("Mask Shrink Grow Resolution Override (MP)", "Optional megapixel override for masked init-image crop scaling.\nSet to 0 to auto-use the model/current generation target.\nFor example, 1.0 is approximately 1024x1024 total pixels.",
            "0", IgnoreIf: "0", Min: 0, Max: 16, ViewMax: 4, Step: 0.05, OrderPriority: -3.695, ViewType: ParamViewType.SLIDER, Group: GroupInitImage, DependNonDefault: MaskShrinkGrow.Type.ID, Examples: ["0", "0.5", "1", "2"]
            ));
        ...
        RefinerPassBeforeUpscale = Register<bool>(new("Refiner Pass Before Upscale", "If enabled and Refiner Upscale is active, run an extra refiner pass before the upscale stage.\nThis is in addition to the normal post-upscale refiner pass.\nUseful when you want extra detail preservation before scaling.",
            "false", IgnoreIf: "false", OrderPriority: 5.1, Group: GroupRefiners, FeatureFlag: "refiners", DoNotPreview: true
            ));
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    /// <summary>List output folders for the current user to target as explicit save destinations.</summary>
    public static List<string> GetUserOutputRootFolderValues(Session session)
    {
        ...
        try
        {
            return [.. Directory.EnumerateDirectories(root, "*", SearchOption.AllDirectories)
                .Select(path => Path.GetRelativePath(root, path).Replace('\\', '/').Trim('/'))
    ...
    {
        if (!FileMediaConversionMeasurement.IsEnabled())
        {
            string originalRoot = Utilities.CombinePathWithAbsolute(Environment.CurrentDirectory, session.User.OutputDirectory);
            (string originalPath, string originalConsoleError, string originalUserError) = WebServer.CheckFilePath(originalRoot, filePath);
            if (originalConsoleError is not null)
            {
                Logs.Error(originalConsoleError);
                throw new SwarmUserErrorException($"Invalid file path {errorContext} - '{filePath}' - {originalUserError}");
            }
            originalPath = UserImageHistoryHelper.GetRealPathFor(session.User, originalPath, root: originalRoot);
            byte[] originalData = null;
            string originalContentType = Utilities.GuessContentType(originalPath);
            string originalPathNorm = Path.GetFullPath(originalPath);
            if (originalData is null && Session.StillSavingFiles.TryGetValue(originalPathNorm, out Task<byte[]> originalCacheData))
            {
                originalData = originalCacheData.Result;
            }
            originalData ??= File.ReadAllBytes(originalPath);
            return $"data:{originalContentType};base64,{Convert.ToBase64String(originalData)}";
        }
        FileMediaConversionMeasurement.CallStart call = FileMediaConversionMeasurement.BeginFileCall();
        string pathNorm = null;
        string source = "none";
        long authorizationUs = 0;
        long waitUs = 0;
        long readUs = 0;
        long encodeUs = 0;
        try
        ...
        {
            FileMediaConversionMeasurement.FailFileCall(call, pathNorm, source, authorizationUs, waitUs, readUs, encodeUs);
            throw;
        }
    }

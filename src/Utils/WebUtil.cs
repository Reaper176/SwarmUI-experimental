...
""");
    ...
    public static async Task<string> NeedLinuxPythonWarn()
    {
        ...
        string pythonVersionRaw = (await tryPyVer("python3.12")) ?? (await tryPyVer("python3.11")) ?? (await tryPyVer("python3.10")) ?? (await tryPyVer("python3")) ?? (await tryPyVer("python"));
        if (string.IsNullOrWhiteSpace(pythonVersionRaw))
        {
            return "Failure to check python version. You must install Python 3.12 or 3.11 before installing SwarmUI.";
        }
        ...
        if (!pythonVersion.StartsWith("Python 3.10") && !pythonVersion.StartsWith("Python 3.11") && !pythonVersion.StartsWith("Python 3.12"))
        {
            if (pythonVersion.StartsWith("Python 3."))
            {
                ...
                return "You have a python version installed, but it is not 3.12 or 3.11. Please install Python 3.12 or 3.11 before installing SwarmUI. 3.10 is relatively stable as well. Older versions will not work, and newer versions will have compatibility issues.";
            }
            return "Python does not appear to be installed on your system. You must install Python 3.12 or 3.11 before installing SwarmUI.";
        }

...
public static class T2IAPI
{
    public static void Register()
    {
        ...
        API.RegisterAPICall(ImageHistoryAPI.AddImageToHistory, true, Permissions.BasicImageGeneration);
        API.RegisterAPICall(ExtractVideoAudio, true, Permissions.BasicImageGeneration);
        API.RegisterAPICall(ImageHistoryAPI.ListImages, false, Permissions.ViewImageHistory);
        API.RegisterAPICall(ImageHistoryAPI.RescanImageMetadata, true, Permissions.ViewImageHistory);
        API.RegisterAPICall(ImageHistoryAPI.ToggleImageStarred, true, Permissions.UserStarImages);
        API.RegisterAPICall(ImageHistoryAPI.ToggleImageHidden, true, Permissions.ViewImageHistory);
        API.RegisterAPICall(ImageHistoryAPI.SetImageRating, true, Permissions.ViewImageHistory);
        API.RegisterAPICall(ImageHistoryAPI.SetImageTags, true, Permissions.ViewImageHistory);
        API.RegisterAPICall(ImageHistoryAPI.SetImageNotes, true, Permissions.ViewImageHistory);
        API.RegisterAPICall(ImageHistoryAPI.OpenImageFolder, true, Permissions.LocalImageFolder);
        API.RegisterAPICall(ImageHistoryAPI.DeleteImage, true, Permissions.UserDeleteImage);
        API.RegisterAPICall(ImageHistoryAPI.BulkMoveImages, true, Permissions.UserDeleteImage);
    ...
    private static async Task<(string inputFile, string temporaryInput, string sourceName)> ResolveVideoSource(Session session, string video, string filename, string action)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    temporaryInput = Path.Combine(Program.TempDir, $"swarm-video-source-{Guid.NewGuid():N}.{extension}");
        ...
        [API.APIParameter("Trim end in milliseconds, or -1 for the end of the video.")] int endMilliseconds = -1)
    {
        ...
        try
        {
            ...
            {
                ["result"] = inputPath,
                ["audio"] = new JObject() { ["path"] = inputPath, ["src"] = src },

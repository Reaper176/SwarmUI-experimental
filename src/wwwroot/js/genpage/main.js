...
function registerNewTool(id, name, genOverride = null, runOverride = null) {
    ...
    let div = SwarmUtil.createDiv(`tool_${id}`, 'tool');
...
function doFeatureInstaller(name, button_div_id, alt_confirm, callback = null, deleteButton = true) {
    ...
    if (buttonDiv) {
        ...
        buttonDiv.appendChild(SwarmUtil.createDiv('', null, 'Installing...'));
    }
    genericRequest('ComfyInstallFeatures', {'features': name}, data => {
        if (buttonDiv) {
            buttonDiv.appendChild(SwarmUtil.createDiv('', null, "Installed! Please wait while backends restart. If it doesn't work, you may need to restart Swarm."));
        }
        reviseStatusBar();
        setTimeout(() => {
        ...
        showError(e);
        if (buttonDiv) {
            buttonDiv.appendChild(SwarmUtil.createDiv('', null, 'Failed to install!'));
...
function imagePromptAddImageData(data, mediaType, fileData = data, fileName = null, replaceTarget = null) {
    ...
    if (existingImage) {
        ...
        let promptImageArea = getRequiredElementById('alt_prompt_image_area');
        let imageContainer = SwarmUtil.createDiv(null, 'alt-prompt-image-container');
        let imageHeader = SwarmUtil.createDiv(null, 'alt-prompt-image-container-header');

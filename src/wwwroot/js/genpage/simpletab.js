...
class SimpleTabBatchWindowManager {

    constructor(scrollContainer, content) {
    ...
    queueUpdate() {
        if (!this.active || this.updateQueued) {
            return;
        }
    ...
    getEntries() {
        return Array.from(this.content.children).filter(entry => entry?.dataset?.src);
    }

    getMediaTarget(entry) {
        let img = entry.querySelector('img.image-block-img-inner');
        if (img) {
            return img;
        }
        return entry.querySelector('video source');
    }

    hydrateEntry(entry) {
        let target = this.getMediaTarget(entry);
        if (!target || !target.dataset.origSrc) {
            return;
        }
        if (target.tagName == 'SOURCE') {
            ...
            target.setAttribute('src', target.dataset.origSrc);
    ...
    updateVisibleWindow() {
        ...
        let visibleStart = scrollLeft - this.bufferPx;
        let visibleEnd = scrollLeft + this.scrollContainer.clientWidth + this.bufferPx;
        for (let entry of entries) {
            let left = entry.offsetLeft;
            let right = left + entry.offsetWidth;
            if (right >= visibleStart && left <= visibleEnd) {
...
class SimpleTab {
    ...
    constructor() {
        ...
        this.batchWrapper = getRequiredElementById('simple_current_image_batch_wrapper');
        ...
        this.histories = {};
        this.batchWindowManager = new SimpleTabBatchWindowManager(this.batchWrapper, this.batchArea);
        this.tabButton.addEventListener('shown.bs.tab', () => {
            this.batchWindowManager.setActive(true);
        });
        getRequiredElementById('toptablist').addEventListener('shown.bs.tab', (e) => {
            this.batchWindowManager.setActive(e.target?.id == 'simpletabbutton');
        });
    }

    stampBatchEntryMedia(entry) {
        let imageTarget = entry.querySelector('img.image-block-img-inner');
        if (imageTarget) {
            let src = imageTarget.getAttribute('src');
            if (src) {
                imageTarget.dataset.origSrc = src;
            }
            return;
        }
        let videoSource = entry.querySelector('video source');
        if (videoSource) {
            let src = videoSource.getAttribute('src');
            if (src) {
                videoSource.dataset.origSrc = src;
            }
        }
    }
    ...
    browserDescribeEntry(workflow) {
        let buttons = [];
        return { name: workflow.name, description: `<b>${SwarmUtil.escapeHtmlNoBr(workflow.name)}</b><br>${SwarmUtil.escapeHtmlNoBr(workflow.data.description ?? "")}`, image: workflow.data.image, buttons: buttons, className: '', searchable: `${workflow.name}\n${workflow.data.description}` };
    }
    ...
    browserSelectEntry(workflow, callback = null) {
        ...
        genericRequest('ComfyReadWorkflow', { name: workflow.name }, (data) => {
            ...
            for (let areaData of [[this.inputsArea, (p) => p.visible && !isParamAdvanced(p), 0],
                ...
                let html = '';
                if (areaData[2] == 0) {
                    html += `<div class="simpletab-workflow-header">${SwarmUtil.escapeHtml(workflow.name)}</div><div class="simpletab-workflow-description">${safeHtmlOnly(workflow.data.description)}</div>`;
                }
                    ...
                    if (groupName != lastGroup) {
                        ...
                        if (param.group) {
                            ...
                            if (param.group.description) {
                                html += `<div class="sui-popover" id="popover_group_${groupId}"><b>${translateableHtml(SwarmUtil.escapeHtml(param.group.name))}</b>:<br>&emsp;${translateableHtml(safeHtmlOnly(param.group.description))}</div>`;
                            ...
                            let toggler = getToggleHtml(param.group.toggles, `simpleinput_group_content_${groupId}`, SwarmUtil.escapeHtml(param.group.name), ' group-toggler-switch', 'doToggleGroup');
                            html += `<div class="input-group input-group-open" id="auto-group-${groupId}"><span id="simpleinput_group_${groupId}" class="input-group-header ${shrinkClass}"><span class="header-label-wrap">${symbol}<span class="header-label">${translateableHtml(SwarmUtil.escapeHtml(param.group.name))}</span>${toggler}${infoButton}</span></span><div class="input-group-content" id="simpleinput_group_content_${groupId}">`;
                        }
                        lastGroup = groupName;
                    }
            ...
            this.batchWindowManager.queueUpdate();
...
class SimpleTabHistory {
    ...
    add(src, metadata, batchId, isLoading) {
        ...
        if (simpleTab.browser.selected == this.workflow) {
            ...
            simpleTab.stampBatchEntryMedia(batch_div);
            if (isLoading) {
        ...
        this.save();
        simpleTab.batchWindowManager.queueUpdate();
        return entry;
    }

...
public partial class WorkflowGenerator
{
    ...
    public int GetLoraScheduleRampKeyframeCount(LoraScheduleKeyframe start, LoraScheduleKeyframe end)
    {
        double range = end.StartPercent - start.StartPercent;
        int keyframes = (int)Math.Ceiling(range * 20) + 1;
    ...
    public JArray CreateHookLorasForConfinement(int confinement, JArray clip, bool scheduledOnly = false)
    {
        ...
        for (int i = 0; i < loras.Count; i++)
        {
            ...
            if (rawSchedule is not null)
            {
                ...
                LoraScheduleParseResult schedule = ParseLoraSchedule(rawSchedule, loras[i]);
                if (schedule.IsInterpolated && !Features.Contains("hook_lora_interpolated_scheduling"))
                {
                    throw new SwarmUserErrorException("LoRA ramp scheduling requires a recent ComfyUI backend with interpolated hook keyframe support.");
                }
                JArray hookKeyframes = CreateHookKeyframesForLoraSchedule(schedule, i);
    ...
    public string CreateKSampler(JArray model, JArray pos, JArray neg, JArray latent, double cfg, int steps, int startStep, int endStep, long seed, bool returnWithLeftoverNoise, bool addNoise, double sigmin = -1, double sigmax = -1, string previews = null, string defsampler = null, string defscheduler = null, string id = null, bool rawSampler = false, bool doTiled = false, bool isFirstSampler = false, bool hadSpecialCond = false, string explicitSampler = null, string explicitScheduler = null, int sectionId = 0, WGNodeData negativeModel = null)
    {
        ...
        if (Features.Contains("variation_seed") && !RestrictCustomNodes)
        {
            inputs[ComfyNodeInputNames.KSampler.VarSeed] = UserInput.Get(T2IParamTypes.VariationSeed, 0, sectionId: sectionId);
            inputs[ComfyNodeInputNames.KSampler.VarSeedStrength] = UserInput.Get(T2IParamTypes.VariationSeedStrength, 0, sectionId: sectionId);
    ...
    public class ImageToVideoGenInfo
    {
        ...
        public void PrepFullCond(WorkflowGenerator g, WGNodeData origSrcImg)
        {
            ...
            else if (VideoModel.ModelClass?.CompatClass?.ID == T2IModelClassSorter.CompatLtxv2.ID)
            {
                ...
                AudioFile referenceAudio = null;
                string referenceAudioParam = "${promptaudios.0}";
                if (g.UserInput.TryGet(T2IParamTypes.PromptAudios, out List<AudioFile> audios) && audios.Count > 0)
                {
                    referenceAudio = audios[0];
                }
                else
                {
                    g.UserInput.TryGet(T2IParamTypes.VideoAudioReference, out referenceAudio);
                    referenceAudioParam = "${videoaudioinput}";
                }
                if (referenceAudio is not null)
                {
                    string audioNode = g.CreateAudioLoadNode(referenceAudio, referenceAudioParam);
            ...
            else if (VideoModel.ModelClass?.CompatClass?.ID == T2IModelClassSorter.CompatMiniMaxH3.ID)
            {
                ...
                string emptyAV = g.CreateNode(ComfyNodeNames.EmptyMiniMaxH3LatentAV, new JObject()
                {
                    [ComfyNodeInputNames.EmptyMiniMaxH3LatentAV.Length] = Frames,
                    [ComfyNodeInputNames.EmptyMiniMaxH3LatentAV.Height] = Height,
                    [ComfyNodeInputNames.EmptyMiniMaxH3LatentAV.Width] = Width,
                    [ComfyNodeInputNames.EmptyMiniMaxH3LatentAV.BatchSize] = g.UserInput.Get(T2IParamTypes.BatchSize, 1)
                });
    ...
    private void CreateImageToVideoInternal(ImageToVideoGenInfo genInfo)
    {
        ...
        if (genInfo.VideoSwapModel is not null)
        {
            ...
            try
            {
                ...
                int steps = UserInput.GetNullable(T2IParamTypes.Steps, swapSectionId, false) ?? genInfo.Steps;
                ...
                samplered = CreateKSampler(swapVideoModel.Path, genInfo.PosCond, genInfo.NegCond, latent.Path, cfg, steps, endStep, 10000, UserInput.Get(T2IParamTypes.Seed, genInfo.Seed + 1, sectionId: swapSectionId, includeBase: false), false, false, sigmin: 0.002, sigmax: 1000, previews: previewType, defsampler: swapDefaultSampler, defscheduler: swapDefaultScheduler, hadSpecialCond: genInfo.HadSpecialCond, explicitSampler: swapExplicitSampler, explicitScheduler: swapExplicitScheduler, sectionId: swapSectionId);
                CurrentMedia = CurrentMedia.WithPath([samplered, 0]);
            }
            finally
            {
    ...
    public JArray CreateConditioningDirect(string prompt, JArray clip, T2IModel model, bool isPositive, string id = null, JArray attachImages = null, int steps = -1)
    {
        ...
        bool wantsSwarmCustom = Features.Contains("variation_seed") && (tokenNormalization != "none" || weightInterpretation != "comfy" || needsAdvancedEncode || (UserInput.TryGet(T2IParamTypes.FluxGuidanceScale, out _) && HasFluxGuidance()) || IsHunyuanVideoSkyreels() || attachImages is not null || minimaxRefs is not null);
        JArray qwenImage;
        if (IsAnima38())
        {
        ...
        else if (IsIdeogram4() || IsKrea2() || (IsBoogu() && isPositive) || IsMageFlow())
        {
            ...
            node = CreateNode("SwarmTextEncodeAdvanced", new JObject()
            {
                ...
                ["lora_hooks"] = CreateDynamicLoraHooks(),
                [ComfyNodeInputNames.ClipTextEncodeAdvanced.Steps] = steps,
        ...
        else if (wantsSwarmCustom)
        {
            node = CreateNode("SwarmTextEncodeAdvanced", new JObject()
            {
                ...
                [ComfyNodeInputNames.ClipTextEncodeAdvanced.TokenNormalization] = tokenNormalization,
                [ComfyNodeInputNames.ClipTextEncodeAdvanced.WeightInterpretation] = weightInterpretation,
                [ComfyNodeInputNames.ClipTextEncodeAdvanced.Images] = attachImages,
    ...
    public JArray CreateConditioningLine(string prompt, JArray clip, T2IModel model, bool isPositive, string id = null, JArray attachImages = null, int steps = -1)
    {
        ...
        // Backup to at least process "<break>" if Swarm nodes are missing.
        if (breaks.Length <= 1)
        {
            return CreateConditioningDirect(prompt, clip, model, isPositive, id, attachImages: attachImages, steps: steps);
        }
        JArray first = CreateConditioningDirect(breaks[0], clip, model, isPositive, id, attachImages: attachImages, steps: steps);
    ...
    private AttentionCouplePlan CreateAttentionCouplePlan(PromptRegion regionalizer, PromptRegion.Part[] parts, HashSet<int> hookConfinements, JArray clip, T2IModel model, bool isPositive, int steps = -1)
    {
        if (!isPositive)
        {
            return null;
        ...
        foreach (PromptRegion.Part part in parts)
        {
            ...
            JArray partCond = CreateConditioningLine(part.Prompt, subClip, model, true, steps: steps);
        ...
        JArray baseCond = CreateConditioningLine(backgroundPrompt, clip, model, true, steps: steps);
    ...
    public WGNodeData CreateSeedVR2Restore(T2IModel seedVrModel, WGNodeData media, WGNodeData decodeVae, long seed, bool isRefiner = false)
    {
        ...
        {
            throw new SwarmReadableErrorException("SeedVR2 restoration requires a newer ComfyUI backend with native SeedVR2 nodes.");
        }
        WGNodeData raw = media.AsRawImage(decodeVae);
        JArray resized = raw.Path;
        string preprocessed = CreateNode(ComfyNodeNames.SeedVR2Preprocess, new JObject()
        {
            [ComfyNodeInputNames.SeedVR2Preprocess.ResizedImages] = resized
        });
        T2IModel priorFinalModel = FinalLoadedModel;
        List<T2IModel> priorFinalModelList = FinalLoadedModelList;
        WGNodeData priorModel = CurrentModel, priorTextEnc = CurrentTextEnc, priorVae = CurrentVae;
        ...
        bool splitLatent = UserInput.Get(ComfyUIBackendExtension.SeedVRSplitLatent, false);
        if (splitLatent && !Features.Contains("seedvr2_temporal_chunking"))
        {
            throw new SwarmReadableErrorException("SeedVR2 temporal chunking requires a newer ComfyUI backend with native SeedVR2 chunk and merge nodes.");
        }
        if (splitLatent)
        {
            int overlap = UserInput.Get(ComfyUIBackendExtension.SeedVRTemporalVideoOverlap, 0);
            string chunked = CreateNode(ComfyNodeNames.SeedVR2TemporalChunk, new JObject()
            {
                [ComfyNodeInputNames.SeedVR2TemporalChunk.Latent] = latent,
                [ComfyNodeInputNames.SeedVR2TemporalChunk.TemporalOverlap] = overlap,
                [ComfyNodeInputNames.SeedVR2TemporalChunk.ChunkingMode] = "auto"
            });
            latent = [chunked, 0];
            chunkOverlap = [chunked, 1];
        }
        string cond = CreateNode(ComfyNodeNames.SeedVR2Conditioning, new JObject()
        {
            [ComfyNodeInputNames.SeedVR2Conditioning.Model] = CurrentModel.Path,
            [ComfyNodeInputNames.SeedVR2Conditioning.VAEConditioning] = latent
        });
        ...
        {
            string merged = CreateNode(ComfyNodeNames.SeedVR2TemporalMerge, new JObject()
            {
                [ComfyNodeInputNames.SeedVR2TemporalMerge.Latents] = sampledLatent,
                [ComfyNodeInputNames.SeedVR2TemporalMerge.TemporalOverlap] = chunkOverlap
            });
            sampledLatent = [merged, 0];
        }
        WGNodeData decoded = encoded.WithPath(sampledLatent).DecodeLatents(CurrentVae, false);
        string post = CreateNode(ComfyNodeNames.SeedVR2PostProcessing, new JObject()
        {
            [ComfyNodeInputNames.SeedVR2PostProcessing.Images] = decoded.Path,
    ...
    public void RunSeedVR2Stage(WGNodeData vaeOverride = null)
    {
        ...
        long seed = UserInput.Get(T2IParamTypes.Seed, UserInput.Get(T2IParamTypes.Seed) + 500, sectionId: T2IParamInput.SectionID_SeedVR, includeBase: false);
    ...
    public JArray CreateConditioning(string prompt, JArray clip, T2IModel model, bool isPositive, string firstId = null, bool isRefiner = false, bool isVideo = false, bool isVideoSwap = false, bool isPixelDecoder = false, JArray attachImages = null, int steps = -1)
    {
        ...
        if (UserInput.Get(ComfyUIBackendExtension.RegionalPromptingMethod, "Standard") == "Attention Couple")
        {
            ...
            PendingAttentionCouplePlan = CreateAttentionCouplePlan(regionalizer, parts, regionalHookConfinements, clip, model, true, steps: steps);

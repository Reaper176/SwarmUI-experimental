...
from .SwarmTextHandling import PROMPT_TEMPLATE_ENCODE_VIDEO_I2V, PROMPT_TEMPLATE_QWEN_IMAGE_EDIT_PLUS, KREA2_TEMPLATE, advanced_encode
...
def legacy_weighted_text(leaves):
    """Render resolved tags for encoders that consume Comfy weight syntax."""
    parts = []
    for leaf in leaves:
        if leaf.lora_hook is not None or leaf.is_break:
            continue
        if leaf.embed is not None:
            text = f"embedding:{leaf.embed} "
        else:
            text = leaf.text or ""
            escaped = []
            i = 0
            while i < len(text):
                if text[i] == "\\" and i + 1 < len(text) and text[i + 1] in "()":
                    escaped.append(text[i:i + 2])
                    i += 2
                    continue
                escaped.append("\\" + text[i] if text[i] in "()" else text[i])
                i += 1
            text = ''.join(escaped)
        parts.append(f"({text}:{leaf.weight})" if leaf.weight != 1.0 else text)
    return ''.join(parts)
...
class SwarmTextEncodeAdvanced:
    ...
    def encode(self, clip, steps: int, prompt: str, width: int, height: int, target_width: int, target_height: int, guidance: float = -1, llama_template = None, clip_vision_output = None, images = None, minimax_refs = None, lora_hooks = None, token_normalization = "none", weight_interpretation = "comfy"):
        append_images = False
        ...
        def encode_leaves(leaves):
            if token_normalization != "none" or weight_interpretation != "comfy":
                # The prompt parser has already resolved schedules and weights. Pass
                # each active leaf to the fork's encoder using Comfy weight syntax.
                cond_arr = advanced_encode(clip, legacy_weighted_text(leaves), token_normalization, weight_interpretation, tokenize)
                for cond in cond_arr:
                    clip.add_hooks_to_dict(cond[1])
                return cond_arr

...
_BLOCK_PATTERN = re.compile(r"(?:diffusion_model\.blocks\.|lora_unet_blocks_)(\d+)")
_LLM_ADAPTER_TOKEN = "diffusion_model.llm_adapter."
...
def infer_source_block_count(state_dict):
    ...
    for key in state_dict:
        for match in _BLOCK_PATTERN.finditer(key):
...
def map_lora_state_dict_to_anima38(state_dict):
    ...
    for key, value in state_dict.items():
        if source_block_count < 52 and _LLM_ADAPTER_TOKEN in key:
            continue
        ...
        mapped_state_dict[_BLOCK_PATTERN.sub(replace_block, key)] = value
